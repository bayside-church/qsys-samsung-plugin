...
- **Tokens persist** across power cycles and reboots and have **no known expiry** — Samsung documents none of this, but no Tizen-2016+ integration (Home Assistant's IP Control and websocket integrations included) implements any refresh, and no expiry reports exist for this protocol. (Reports of tokens lapsing after weeks apply to the 2014–2015 H/J-series *encrypted* remote, a different protocol.) What revokes a token: factory reset, disabling IP Remote, removing the device from the TV's device list, **a new pairing from any client** (one token per TV, §7.9), and occasionally a firmware update that changes the API.
...
| 2018 set (NU6900), websocket-only | ✔ Pairing from its subnet; `KEY_POWER` → standby; `KEY_POWER` again wakes it **only while its Wi-Fi is still up (~2 min after standby, measured)**; after that the radio sleeps and only Wake-on-LAN works. WoL needs **repeated unicast** (3 rounds, 1 s apart): single packets and broadcast-only bursts did not wake it; Q-SYS `UdpSocket` works for this bound or unbound. *Power On with Mobile* was on throughout. No `PowerState` in `/api/v2/` on this model. `KEY_POWEROFF` is ignored; `KEY_POWER` is the only power key and it toggles. An incoming websocket connection (pairing) wakes the set from standby. |
